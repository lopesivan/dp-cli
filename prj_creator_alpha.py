...
def show_menu() -> str:
    ...
    table = Table(show_header=True, header_style="bold white on dark_blue", expand=True)
...
def run_step(description: str, func, *args, **kwargs) -> bool:
    ...
    try:
        ...
        console.print(f"  [bold red]✗[/] {description} — falhou (código {e.returncode})")
        if e.stdout:
...
def summary_panel(name: str, path: Path, lang: str, color: str, extra_lines: list[str]) -> None:
    ...
    console.print(Panel(body, title="🎉 Projeto criado com sucesso!", border_style=color, expand=False))
...
'''
    ...
    ok = run_step("Inicializando módulo Go (go mod init mymodulo)", run_cmd, ["go", "mod", "init", "mymodulo"], path)
...
def create_cs_dotnet(name: str) -> None:
    ...
    run_step("Gerando .csharpierrc.json", write_file, path / ".csharpierrc.json", csharpierrc)
...
'''
    ...
    run_step(f"Gerando {name}.csproj", write_file, path / f"{name}.csproj", csproj)
    run_step("Gerando Makefile", write_file, path / "Makefile", makefile)
    run_step("Gerando .csharpierrc.json", write_file, path / ".csharpierrc.json", csharpierrc)
...
def create_kotlin(name: str) -> None:
    ...
    gradle_home = next((g for g in gradle_home_candidates if Path(g).exists()), gradle_home_candidates[0])
    ...
    ok = run_step(
        "Rodando gradle init --type kotlin-application",
        run_cmd,
