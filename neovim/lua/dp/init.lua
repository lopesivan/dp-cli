...
local function project_root()
    ...
    return vim.fs.root(start, { ".git", "pyproject.toml", "Makefile" }) or vim.fn.getcwd()
end
...
local function parse_registry(output)
    ...
    for line in output:gmatch("[^\r\n]+") do
        ...
        if language then
            registry[language] = vim.split(items, ",", { plain = true, trimempty = true })
...
local function get_registry(callback)
    vim.system({ command(), "--list" }, { cwd = project_root(), text = true }, function(result)
        vim.schedule(function()
            if result.code ~= 0 then
                notify(result.stderr ~= "" and result.stderr or "Não foi possível listar os patterns.", vim.log.levels.ERROR)
                return
            end
...
function M.edit_config()
    ...
    notify("key.yaml ainda não existe. Execute :DP primeiro.", vim.log.levels.WARN)
end
...
function M.setup(options)
    ...
    if M.options.keymap then
        vim.keymap.set("n", M.options.keymap, M.select_and_generate, {
